...
const Room3D = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 md:py-10 bg-[#F8FAFC]">
      {/* Header */}
      <div className="flex flex-col gap-4 mb-6 md:mb-8">
        <div>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 tracking-tight text-gradient bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          <button
            ...
            className="bg-white border border-slate-200 text-slate-700 px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl font-bold hover:bg-slate-50 transition-all shadow-sm flex items-center gap-2 text-xs sm:text-sm"
          >
            Browse Products
          </button>
          <button
          ...
          {inventoryItems.some(i => i.hasModel) && (
            <button
              ...
              className="bg-green-600 text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl font-bold hover:bg-green-700 transition-all shadow-md flex items-center gap-2 text-xs sm:text-sm"
            >
              <Smartphone size={16} /> AR
            </button>
          )}
        </div>
      </div>
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {rooms.map((room, idx) => (
          <button
            ...
            className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-full text-xs font-bold transition-all whitespace-nowrap border ${selectedRoomIndex === idx
    ...
    <div className="relative group">
      ...
      {isDemo && (
        <div className="absolute top-4 sm:top-6 right-4 sm:right-6 bg-amber-500/90 backdrop-blur text-white text-[9px] sm:text-[10px] px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl font-black uppercase tracking-widest shadow-lg">
          Demo Mode
        </div>
      )}
    </div>
    ...
    <div className="bg-white p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6 md:mb-8">
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-slate-800">Room Inventory</h2>
        ...
        <div className="flex -space-x-2">
          {(isDemo ? demoItems : inventoryItems).slice(0, 5).map((item, i) => (
            <div key={item.id} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center overflow-hidden shadow-sm">
              {item.thumbnail ? (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
        {(isDemo ? demoItems : inventoryItems).map((item) => (
          <div key={item.id} className="group bg-slate-50/50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-transparent hover:border-[#635465]/10 hover:bg-white hover:shadow-md transition-all relative">
            {!isDemo && (
              <button
                onClick={() => handleRemoveProduct(item.relId)}
                className="absolute top-2 right-2 p-1 sm:p-1.5 bg-red-50 text-red-500 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500 hover:text-white z-10"
              >
                <Trash2 size={12} />
              </button>
            )}
            <div className="w-full aspect-square bg-slate-200 rounded-lg sm:rounded-xl mb-2 sm:mb-3 flex items-center justify-center overflow-hidden relative">
              {item.thumbnail ? (
                <img src={item.thumbnail} className="w-full h-full object-cover transition-transform group-hover:scale-110" alt={item.name} />
              ) : (
                <span className="text-2xl">📦</span>
              )}
              {!item.hasModel && !isDemo && (
                <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] flex flex-col items-center justify-center p-2 sm:p-4 text-center">
                  <BoxIcon size={16} className="text-white/50 mb-1 sm:mb-2" />
                  <span className="text-[7px] sm:text-[8px] font-black text-white uppercase tracking-tighter">No 3D Model</span>
                </div>
              )}
            </div>
            <p className="text-[10px] sm:text-xs font-bold text-slate-700 truncate">{item.name}</p>
            <div className="flex justify-between items-center mt-1">
              <p className={`text-[8px] sm:text-[10px] font-medium uppercase tracking-wider ${item.hasModel || isDemo ? 'text-green-500' : 'text-slate-400'}`}> 
              ...
              {item.hasModel && (
                <button
                  ...
                  className="p-1 sm:p-1.5 bg-slate-100 text-slate-600 rounded-lg hover:bg-[#635465] hover:text-white transition-all shadow-sm"
                  title="View in AR"
                >
                  <Smartphone size={10} />
                </button>
        ...
        {!isDemo && inventoryItems.length === 0 && (
          <div className="col-span-full py-8 sm:py-12 text-center bg-slate-50 rounded-2xl sm:rounded-3xl border border-dashed border-slate-200">
            <BoxIcon size={32} className="mx-auto mb-3 sm:mb-4 text-slate-300" />
            <p className="font-bold text-slate-500 text-sm">Your room is empty</p>
            <p className="text-xs text-slate-400 mt-1 mb-4 sm:mb-6">Browse products and click the 🏠 icon to add them here</p>
            <button onClick={() => navigate('/products')} className="bg-[#635465] text-white text-xs font-bold px-5 sm:px-6 py-2 sm:py-2.5 rounded-xl hover:bg-[#524554] transition-all">Browse Products</button>
          </div>
        )}
      </div>
    </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 sm:p-6">
          <div className="bg-white rounded-2xl sm:rounded-[2rem] w-full max-w-md p-6 sm:p-8 shadow-2xl">
            ...
            <form onSubmit={handleCreateRoom} className="space-y-4 sm:space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border-none rounded-xl sm:rounded-2xl text-slate-800 focus:ring-2 focus:ring-[#635465]/20 outline-none font-bold text-sm"
                  autoFocus
                />
              </div>
              <button
                type="submit"
                className="w-full bg-[#635465] text-white py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black shadow-lg text-sm sm:text-base"
              >
                Create Room
              </button>
            </form>
      ...
      {/* AR Modal Overlay */}
      {showAR && (
        <div className="fixed inset-0 z-[10000] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 md:p-10 overflow-hidden animate-in fade-in duration-300">
          <div className="w-full max-w-4xl max-h-[90vh] bg-white rounded-2xl sm:rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
